...
public class Pacman extends Thread
{
    ...
    synchronized public void move()
    {
        ...
        {
            if(this.req_dx == -1 && this.req_dy == 0 && !roundness[0])
            {
                this.setPacman_dx(this.getReq_dx());
                this.setPacman_dy(this.getReq_dy());
            }
            if(this.req_dx == 0 && this.req_dy == -1 && !roundness[1])
            {
                this.setPacman_dx(this.getReq_dx());
                this.setPacman_dy(this.getReq_dy());
            }
            if(this.req_dx == 1 && this.req_dy == 0 && !roundness[2])
            {
                this.setPacman_dx(this.getReq_dx());
                this.setPacman_dy(this.getReq_dy());
            }
            if(this.req_dx == 0 && this.req_dy == 1 && !roundness[3])
            {
                this.setPacman_dx(this.getReq_dx());
                this.setPacman_dy(this.getReq_dy());
            }

            if(this.getPacman_dx() == -1 && this.getPacman_dy() == 0 && roundness[0])
            {
                this.setPacman_dx(0);
                this.setPacman_dy(0);
            }
            if(this.getPacman_dx() == 0 && this.getPacman_dy() == -1 && roundness[1])
            {
                this.setPacman_dx(0);
                this.setPacman_dy(0);
            }
            if(this.getPacman_dx() == 1 && this.getPacman_dy() == 0 && roundness[2])
            {
                this.setPacman_dx(0);
                this.setPacman_dy(0);
            }
            if(this.getPacman_dx() == 0 && this.getPacman_dy() == 1 && roundness[3])
            {
                this.setPacman_dx(0);
                this.setPacman_dy(0);
            }
    ...
    synchronized public void setRoundness(boolean[] roundness)
    {
